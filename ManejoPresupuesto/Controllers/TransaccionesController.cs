...
namespace ManejoPresupuesto.Controllers
{
    public class TransaccionesController : Controller
    {
        ...
        private async Task<IEnumerable<SelectListItem>> ObtenerCuentas(int idUsuario)
        {
            ...
            var resultado = cuentas.
                Select(x => new SelectListItem(x.NombreCuenta, x.Id.ToString())).ToList();

            var opcionPorDefecto = new SelectListItem("-- Seleccione una cuenta --", "0", true);
        ...
        private async Task<IEnumerable<SelectListItem>> ObtenerCategorias(int idUsuario, TipoTransaccion IdTipoTransaccion)
        {
            ...
            var resultado =  categorias.
                Select(x => new SelectListItem(x.NombreCategoria, x.Id.ToString())).ToList();
