...
namespace ManejoPresupuesto.Service
{
    public class TipoCuentaRepository : ITipoCuentasReposritory
    {
        ...
        public async Task<bool> Existe(string nombreTipoCuenta, int idUsuario, int id = 0)
        {
            using var connection = new SqlConnection(_connectionString);
            var existe = await connection.QueryFirstOrDefaultAsync<int>
                                            ...
                                            AND Id <> @id;",
                                            new {nombreTipoCuenta, idUsuario, id});
        ...
        public async Task<IEnumerable<TipoCuenta>> Obtener(int idUsuario, PaginacionViewModel paginacion)
        {
            using var connection = new SqlConnection(_connectionString);
            return await connection.QueryAsync<TipoCuenta>
                                        (@$"SELECT Id, IdUsuario, NombreTipoCuenta
                                        FROM TipoCuentas 
                                        WHERE IdUsuario = @idUsuario
                                        ORDER BY NombreTipoCuenta
        ...
        public async Task<int> Contar(int idUsuario)
        {
            ...
            return await connection.ExecuteScalarAsync<int>(
                "SELECT COUNT(*) FROM TipoCuentas WHERE IdUsuario = @IdUsuario", new { idUsuario }
                );
        }
